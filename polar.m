function hpol = polar(theta,rho,line_style,npokeangles)
...
%   Hacked by Zepu Zhang to allow more user control.
%
%   NPOKEANGLES should be even.
...
% plot spokes
	if nargin < 4 | isempty(npokeangles)
		npokeangles = 12;
	end
	if mod(npokeangles, 2) > 0
		npokeangles = npokeangles + 1;
	end
	th = (1:(npokeangles/2)) * 2*pi / npokeangles;
...
% annotate spokes in degrees
    rt = 1.1*rmax;
    for i = 1:length(th)
        text(rt*cst(i),rt*snt(i),int2str(i*360/npokeangles),...
             'horizontalalignment','center',...
             'handlevisibility','off');
        if i == length(th)
            loc = int2str(0);
        else
            loc = int2str(180+i*360/npokeangles);
        end
